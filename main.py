...
def detect_backends():
    """检测已配置的邮箱后端"""
    backends = []
    if getattr(config, 'EMAIL_DOMAIN', '') and getattr(config, 'EMAIL_API_URL', '') and (getattr(config, 'EMAIL_API_TOKEN', '') or getattr(config, 'EMAIL_ADMIN_PASSWORD', '') or getattr(config, 'EMAIL_SITE_PASSWORD', '')):
        backends.append({
            'name': 'cloudflare',
...
def main():
    ...
    provider_name = getattr(config, 'EMAIL_PROVIDER', None) or choose_backend(backends)

    count = int(getattr(config, 'RUN_COUNT', 10) or 10)
    threads = int(getattr(config, 'RUN_THREADS', 1) or 1)
